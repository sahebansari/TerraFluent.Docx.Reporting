...
Publishing can be done from GitHub Actions by pushing to `main` or `master`, pushing a version tag such as `v1.2.2`, or by running the `CI` workflow manually with `publish=true`.
